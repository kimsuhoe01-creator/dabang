(() => {
  ...
  const copy = {
    ko: { button:"음성 주문", guide:"직원에게 말하듯 메뉴판을 보면서 편하게 말씀해 주세요.", connecting:"음성 주문을 연결하고 있습니다…", listening:"듣는 중 · 취소", listeningGuide:"듣고 있습니다. 메뉴판을 보면서 취소나 변경도 편하게 말씀해 주세요.", finish:"음성 주문 보내기", processing:"말씀하신 주문을 정리하고 있습니다…", retry:"다시 시도", continueAnswer:"답변 이어 말하기", empty:"말씀하신 주문을 듣지 못했습니다.", unavailable:"지금은 음성 주문 서버에 연결할 수 없습니다. 잠시 후 다시 시도해 주세요.", timeout:"주문 정리가 오래 걸리고 있습니다. 다시 말씀해 주세요.", failed:"음성 주문을 처리하지 못했습니다. 다시 말씀해 주세요.", cartBusy:"음성 주문은 빈 장바구니에서 시작해 주세요.", mic:"마이크 사용을 허용해 주세요.", mockLabel:"미리보기용 음성 주문", cartTitle:"말씀하신 주문을 확인해 주세요", cartCaption:"주문 내용이 맞으면 아래 주문 전송 버튼을 눌러 주세요.", cartRetry:"다시 말하기", cartSend:"맞아요 · 주문 전송", cartNote:"버튼을 누르면 이 내용 그대로 POS로 전송됩니다. 메뉴·옵션·수량을 확인해 주세요." },
  ...
  function updateVoiceLanguage(){ setPhase(voice.phase,["error","clarify"].includes(voice.phase)?el("voiceGuide").textContent:""); }
  ...
  function openVoiceOrder(opener){
    ...
    if(voice.phase==="idle")voice.followUpContext=null;
  ...
  function restartVoiceOrderFromCart(){
    if(orderSending)return;deactivateDialogLayer(el("cartModal"),false);window.voiceCartReviewActive=false;cartItems=[];pendingOrderPayload=null;voice.followUpContext=null;recalculateCart();setTimeout(()=>openVoiceOrder(el("voiceOrderButton")),0);
  }
