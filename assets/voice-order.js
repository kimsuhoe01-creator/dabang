(() => {
  ...
  async function completeRealtimeResponse(response){
    ...
    if(lookupCall){
      ...
      setPhase("processing",t().lookup);sendFunctionResult(lookupCall.call_id,publishedMenuLookup(args));voice.assistantText="";requestRealtimeResponse();return;
    }
  ...
  function publishedMenuLookup(args){
    ...
    if(requested==="ALL_CATEGORIES")return{ok:true,status:"completed",kind:"categories",exactCategoryNames:categories.map(category=>localizedNames(category.names)||category.name),instruction:"The lookup is complete. Answer now using only these exact category names."};
    const category=categories.find(row=>row.name===requested||Object.values(row.names||{}).includes(requested));
    if(!category)return{ok:true,status:"completed",kind:"items",exactMenuNames:[],exactCategoryNames:categories.map(row=>localizedNames(row.names)||row.name),instruction:"The lookup is complete. The requested category is not published. Do not invent a name."};
    ...
    return{ok:true,status:"completed",kind:"items",exactCategoryName:localizedNames(category.names)||category.name,exactMenuNames,instruction:"The lookup is complete. Answer now. Copy only these exact menu names verbatim. Do not add, rename, translate, or combine items."};
  }
