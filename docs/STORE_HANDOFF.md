...
## 2026-09-16 닭갈비 한판 신메뉴 출시

- CUKCUK에 코드 `DKG`, 판매가 378,000동(VAT 8% 포함), 단위 `Set`, 주방 `bếp`으로 등록했다. 메뉴명은 `닭갈비 한판 | Dakgalbi gà xào cay | 辣炒鸡拼盘 | Dakgalbi Chicken Platter`이고 신메뉴 카테고리에 둔다.
- 태블릿은 닭갈비가 들어 있는 신메뉴 카테고리를 첫 진입 카테고리로 올리고, 닭갈비를 일반 카드 대신 첫 화면을 채우는 대형 6초 무음 반복 영상 카드로 표시한다. 동작 줄이기 환경과 영상 로딩 실패 시에는 사용자가 제공한 실제 원본 음식 사진이 보인다. 영상 프레임을 포스터로 쓰지 않는다.
- 대형 카드를 누르면 기존 메뉴 선택·장바구니·CUKCUK 주문 전송 흐름을 그대로 사용한다. 장바구니와 주문내역에도 닭갈비 포스터가 표시된다.
- 영상은 1280×720 H.264, 24fps, 6초, 무음, faststart, 약 1.86MB이며 설치형 태블릿 캐시는 `dabang-tablet-v47`이다. 테이블·언어 선택 후 영상 재생을 한 번 더 명시적으로 요청한다.
...
- 매일 19:00 이후 삿포로 1+1이 자동 주문 불가가 될 때 일반 품절 문구 대신 `오늘의 1+1 행사 종료 / 매일 19:00까지 이용하실 수 있어요. 내일 다시 만나요!`를 표시한다.
- 같은 의미의 베트남어·중국어·영어 문구도 함께 제공한다. 19:00 전에는 기존 해피아워 안내만 보이며, 재고 부족 등 다른 사유의 품절에는 일반 품절 안내가 유지된다.
- 매장 시간 19:02 공개 안전 미리보기에서 삿포로 1+1 카드가 비활성화되고 새 제목·설명이 정확히 표시되는 것을 확인했다. 실제 주문은 보내지 않았다.
- 기능 커밋: `e81f5c2` (`Show Sapporo event-ended notice`), Cloudflare Worker 버전: `942eb0fb-5a16-40a9-bbda-a7af656743f1`, 태블릿 캐시: `dabang-tablet-v45`.
- 검증: Worker/UI 전체 테스트 99개 통과, 메뉴 이미지 감사 112개·누락 0, `git diff --check`, 공개 `/health`, 19:00 자동 차단과 전용 문구를 확인했다.
- 안전 미리보기: https://kimsuhoe01-creator.github.io/dabang/tablet-preview.html?preview=1&deploy=e81f5c2

## 2026-09-05 매장 GPT 태블릿 숨김·다시 표시 연결

- 매장 계정의 비공개 `다방치킨 매장 운영 GPT`에 태블릿 전용 메뉴 숨김, 다시 표시, 최근 변경 이력 조회를 추가했다. 기존 매출 조회, 메뉴 검색·상태 확인, 품절·판매 재개도 그대로 유지한다.
- `안 보이게`, `숨겨`, `메뉴에서 빼`, `삭제해`라는 요청은 CUKCUK나 구글 원본 삭제로 보내지 않고 태블릿 전용 숨김으로 처리한다. 숨긴 메뉴는 화면에서 사라지고 서버의 최종 주문 검증에서도 차단된다.
- 품절과 숨김은 독립 상태다. 다시 표시해도 별도 품절은 유지되고, 판매 재개해도 태블릿 숨김은 유지되므로 한 작업이 다른 안전 상태를 실수로 풀지 않는다.
- 숨김 적용 전에 장바구니에 들어 있던 메뉴는 다음 상태 갱신 때 제거하고 한국어·베트남어·중국어·영어로 안내한다. 상태 서버가 잠시 실패하면 마지막으로 받은 숨김 상태를 유지한다.
- 새 GPT 작업은 `hideDabangMenuFromTablet`, `showDabangMenuOnTablet`, `getDabangMenuChangeHistory`이며, 쓰기 작업은 메뉴 검색·정확한 이름 확인·고유 요청번호·사용자 확인을 거친다. 기존 전용 인증키와 비공개 공개 범위는 변경하지 않았다.
- 실제 매장 GPT에서 `쥬시큘 태블릿에서 안 보이게 해 줘`를 실행해 성공했다. 공개 태블릿 안전 미리보기에서 음료 메뉴가 6개로 표시되고 쥬시큘이 보이지 않는 것을 확인했다. 최근 변경 이력 조회도 같은 숨김 기록을 반환했다. CUKCUK·구글 원본과 주문은 변경하지 않았다.
- 기능 커밋: `e4da652` (`Add tablet menu visibility controls`), Cloudflare Worker 버전: `7f83c17a-b6a9-4003-a8bd-f2042c7096fd`, OpenAPI 버전: `2.1.0`, 태블릿 캐시: `dabang-tablet-v44`.
- 검증: Worker 전체 테스트 98개 통과, 메뉴 이미지 감사 112개·누락 0, `git diff --check`, 공개 `/health`, 새 OpenAPI 경로 3개, 무인증 401, 태블릿 숨김·주문 차단 상태를 확인했다.
- 안전 미리보기: https://kimsuhoe01-creator.github.io/dabang/tablet-preview.html?preview=1&deploy=e4da652

## 2026-09-04 매장 GPT 품절·판매 재개 연결

- 기존 매장 GPT가 쓰던 인증 연결을 그대로 사용해 메뉴 검색, 현재 품절 상태 조회, 품절/일시 판매 중지, 판매 재개를 할 수 있게 했다.
- 개인 계정의 새 매장 GPT에는 별도 전용 인증키를 추가해 기존 매출 조회 GPT의 인증을 바꾸거나 끊지 않도록 했다. 저장소와 문서에는 새 키의 SHA-256 해시만 두고 평문은 남기지 않는다.
- GPT는 메뉴 이름을 먼저 검색한다. 같은 이름의 M/L 메뉴처럼 후보가 여러 개면 임의로 고르지 않고 매니저에게 정확히 어느 메뉴인지 묻는다.
- 품절은 `오늘 영업 종료까지`, `지정 시각까지`, `별도 안내 전까지` 중 하나로 저장할 수 있다. 자동 해제 시각은 베트남 매장 시간 기준이다.
- 판매 재개는 GPT가 만든 수동 품절만 해제한다. CUKCUK 원본 품절, 시간 예약 차단, 카테고리 휴무는 우회하지 않는다.
- 저장 요청에는 현재 메뉴의 정확한 이름과 고유 요청번호가 필요하다. 메뉴 데이터가 바뀌었거나 같은 요청이 재전송돼도 다른 메뉴를 잘못 바꾸거나 중복 처리하지 않는다.
...
- 전용 인증키 커밋: `43223b4` (`Add dedicated store GPT token`), GPT Actions 스키마 호환 커밋: `16335a0` (`Make GPT action schema importer compatible`)
- Cloudflare Worker 버전: `b005b01c-c8a5-4abf-a622-a595d460d931`
- 검증: Worker 전체 테스트 91개, 메뉴 이미지 감사 112개·누락 0, `git diff --check`, Worker 배포 전 문법·번들 검사를 통과했다. 공개 서버에서 `/health` 정상, OpenAPI 2.0.0, 무인증 요청 401, 현재 수동 품절 0개를 확인했다. 실제 메뉴 상태와 CUKCUK/POS 주문은 변경하지 않았다.
- 실제 사용 대상은 사장님 개인 ChatGPT 계정이 아니라 매장 전화번호의 Google 계정으로 가입한 별도 유료 ChatGPT 계정이다. 개인 계정의 이름 없는 GPT 초안은 생성·공유하지 않았고, 매장 계정 로그인 후 해당 계정에서 새 GPT를 구성해야 한다.
...
- 기존에 검게 점멸하며 주 버튼처럼 보이던 `듣는 중 · 취소`는 움직임과 빛 효과를 없앤 작은 회색 `듣기 취소` 버튼으로 낮췄다. 버튼 순서는 안내 → 말하기 완료 → 듣기 취소 → 간격 → 장바구니다.
- 첫 대기 화면의 `직원에게 말하듯 메뉴판을 보면서 편하게 말씀해 주세요`는 오른쪽 정렬하고 빨간 화살표로 바로 옆 `음성 주문` 버튼을 가리키게 했다. 청취·추가 질문 중에는 화살표를 감춘다.
- 베트남어·중국어·영어의 취소 문구도 `청취 취소` 의미로 맞췄다. 설치형 태블릿 캐시는 `dabang-tablet-v35`, 음성 자산은 `20260831-v10`이다.
- 기능 커밋은 `f70bf27` (`Clarify voice listening controls`)이다. 1280×800 안전 미리보기에서 실제 표시 순서·색상·너비를 확인했고, 실제 마이크·POS 주문은 실행하지 않았다.
- 전체 회귀 테스트 68개, 메뉴 이미지 감사 112개·누락 0, JavaScript 문법 검사와 `git diff --check`를 통과했다.
- 안전 미리보기: https://kimsuhoe01-creator.github.io/dabang/tablet-preview.html?preview=1&deploy=f70bf27
- 매장 실주문 주소: https://kimsuhoe01-creator.github.io/dabang/tablet-preview.html?source=store-qr&deploy=f70bf27
...
## 테이블 주문내역 D-6 현장 수정 (2026-08-31)

- 작업 컴퓨터: `DESKTOP-NQAD8O7`
- 현장 POS에서 D-6이 사용 중이며 총액 714,200동인 상태인데, 태블릿 주문내역 창은 0건으로 표시되는 문제를 재현했다.
- 계산 직전 읽기 전용 진단에서 CUKCUK 주문 목록은 D-6의 최신 주문을 `Status: 4`, 금액 714,200동, 시각 `2026-08-31T22:03:22+07:00`으로 반환했다. CUKCUK에서 상태 4가 테이블 사용 중에도 나타날 수 있으므로 상태값만으로 계산 완료를 판단하면 안 된다.
- 주문내역 조회는 이제 실시간 테이블 목록의 `IsAvailable`/`Status`로 사용 중 여부를 먼저 확인한다. 사용 중이면 같은 테이블의 가장 최신 주문을 표시하고, 계산 후 테이블이 비면 상태 4의 과거 주문을 표시하지 않는다.
- 오래된 캐시 화면이 테이블 이름 헤더를 보내지 않아도 테이블 UUID로 CUKCUK 실시간 테이블 이름을 찾아 조회한다.
- 계산 후 D-6이 비어 있는 상태에서 실서버 조회가 200 응답, 0건, 약 1.65초인 것을 확인했다. 계산 전 714,200동은 진단 로그로 포착했으며, 진단 로그 코드는 최종 배포에서 제거했다.
- 주문 생성·수정·결제 API는 호출하지 않았고 조회만 수행했다.
- 최종 Worker 버전: `95617384-a75c-478e-b98d-33d8c20758f7`
- 코드 커밋: `ef03cce` (`Fix occupied POS table history lookup`)
- 검증: Worker 전체 테스트 73개 통과, 메뉴 112개 이미지 감사 통과(누락·고아 0), `git diff --check` 통과.
- 다음 현장 확인: 사용 중인 다른 테이블에서 영수증 아이콘을 눌러 현재 총액과 메뉴 줄이 POS와 같은지만 1회 확인한다. 실제 주문 전송은 필요 없다.

## 스페이스 피자 금일 휴무 처리 (2026-09-03)

- 작업 컴퓨터: `DESKTOP-NQAD8O7`
- 베트남 매장 날짜 `2026-09-03`에만 `스페이스 피자` 카테고리의 8개 메뉴와 `세트` 카테고리의 `치피 세트 (M)/(L)`를 주문 불가로 처리한다.
- 피자 카테고리에는 메뉴별 품절 덮개를 나열하지 않고 `스페이스 피자 금일 휴무 / 오늘은 피자 메뉴를 주문하실 수 없습니다.` 안내 한 장만 표시한다. 안내는 KO/VI/ZH/EN을 모두 제공한다.
- 치피 세트 2개는 세트 카테고리에서 숨긴다. 공개 화면, 오래 열린 화면, 음성 주문 카탈로그, 최종 주문 검증에 같은 서버 판매상태를 적용한다.
- 베트남 시간 `2026-09-04 00:00`부터 휴무 처리가 자동 해제되어 피자와 치피 세트가 다시 정상 노출된다. CUKCUK 원본 메뉴의 판매상태는 바꾸지 않았다.
- 사포로 1+1은 기존 요청대로 매일 `19:00`부터 서버에서 자동 주문 불가이며, 안내문도 `매일 19:00까지 1+1`로 정정했다.
- Cloudflare Worker 버전: `5893a196-7978-4f70-8fb8-ff817e598df5`
...
## 짭코바 1·2·3단계 선택 복구 (2026-09-16)

- `(KX02) 짭코바`와 `(KX14) 짭코바 반마리` 모두 매운 단계를 `1단계 → 2단계 → 3단계` 순서로 표시하고, 주문 전에 정확히 하나를 반드시 선택하게 했다.
- 3단계는 CUKCUK 실제 Addition ID `90f5ee61-9405-4725-a244-24b23222ee4f`를 사용한다. 태블릿 표시용 가짜 옵션을 만들지 않았다.
- 두 메뉴가 공용 옵션 템플릿을 쓰던 구조를 메뉴별 템플릿으로 분리했다. 따라서 짭코바 두 메뉴에는 1·2·3단계를 유지하면서, 같은 공용 매운 단계 그룹을 쓰는 디진다 돈까스는 기존 1·2단계만 유지한다.
- CUKCUK 일반 상품 상세에는 짭코바의 온라인 주문용 공용 추가 옵션이 7개 남아 있지만 실제 테이블 QR에는 메뉴별 4개만 노출된다. 자동 동기화는 이 두 메뉴에 한해 실제 테이블 QR 상세를 읽도록 바꿨고, 예상 카테고리·옵션 ID가 달라지면 잘못 게시하지 않고 실패하도록 했다.
- `(KX02)` 추가 옵션은 밥·통마늘·모짜렐라·우동, `(KX14)` 추가 옵션은 로제 짭코바·통마늘·모짜렐라·우동으로 각각 유지한다.
- 전체 Worker/UI 테스트 109개, 실시간 테이블 QR 상세 대조, 메뉴 이미지 113개 감사를 통과했다. 3단계가 실제 CUKCUK 주문 상세로 변환되는 것과 미선택 주문이 거절되는 것을 자동 테스트로 확인했으며 실제 POS 주문은 보내지 않았다.
